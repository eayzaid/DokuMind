...
DokuMind deliberately separates concerns across three microservices, each chosen for its optimal fit to the task:
...
\begin{tikzpicture}[node distance=2.5cm, every node/.style={font=\sffamily\small}]
  ...
  \draw[{Latex[length=5pt]}-{Latex[length=5pt,open]}, thick, color=DeepRose!70]
    (usr.south) -- node[right,font=\sffamily\scriptsize,color=DarkEarth]{1 : 0..1} (rt.north);
